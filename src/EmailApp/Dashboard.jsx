...
// List of languages English can be translated to
 const languages = [
  ...
  const handleSendEmails = async () => {
    ...
    const emailList = Object.keys(emailObject);
    const url = "https://bulkemailapplanguagebackend.onrender.com/api/email/send"; // Replace with your actual backend URL
    const payload = { subject, body: message, emailList, language };
    console.log(payload)
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[100%]  pt-18 w-[100%] dark:bg-gray-700 bg-gray-100 dark:text-white">
      <div className="w-[89%]  h-[98%] max-h-[90%] overflow-y-scroll p-6 rounded-lg shadow-md bg-white dark:bg-gray-800">
        ...
        <input
          ...
          className="w-full p-4  h-[10%]  mb-4 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
        />
    ...
    <option className=' text-black dark:text-white ' key={i} value={lang.value}>{lang.label}</option>
            ))}
          </select>
        </div>
